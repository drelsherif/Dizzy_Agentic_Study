#!/usr/bin/env python3
"""
DizzyCTA Agentic Pipeline v14.0.0 — Cleaned for Public Release
=============================================================
...
synthetic set again after upgrading to confirm all three are resolved):
  ...
  2. CTH-indeterminate resolution was one-directional. If MRI came back clean,
     a CTH Indeterminate=Yes case stayed permanently unresolved instead of
     being settled negative. qc_check_cth_indeterminate_resolved_by_mri() now
     resolves Indeterminate=Yes -> No when a technically diagnostic follow-up
     MRI finds neither an acute infarct nor a hemorrhage/other abnormality of
     concern (a Limited/Nondiagnostic MRI still does not resolve it, since it
     cannot reliably rule anything out).
...
def qc_check_cth_indeterminate_resolved_by_mri(cth: dict, mri: dict, flags: list) -> dict:
    ...
    (Indeterminate=Yes) is deliberately left unresolved on CTH because the follow-up MRI
    is expected to settle it. If the MRI subsequently confirms a real, definite finding
    (an acute infarct, or a hemorrhage), retroactively resolve the CTH call instead of
    leaving it as a permanent "No Acute" that a reviewer has to manually reconcile against
    a positive MRI in the same case.

    v14.0.0 FIX: this previously only resolved toward a POSITIVE finding. If MRI came back
    clean, an Indeterminate=Yes case was left permanently unresolved forever, even though a
    negative, technically diagnostic follow-up MRI is just as much a real answer to "is
    there an acute finding" as a positive one. Found via 15-case synthetic test set: a case
    built specifically to test negative resolution stayed Indeterminate=Yes indefinitely.
    Both directions are now handled symmetrically.
    """
    ...
    mri_quality = str(mri.get('MRI_Technical_Quality', '')).strip()
    if mri_acute or mri_hemorrhage:
        ...
        flags.append(f"CTH_Acute_Finding resolved No Acute->Acute (Indeterminate->No): "
                      f"follow-up MRI confirmed {'hemorrhage' if mri_hemorrhage else 'acute infarct'}")
        cth['CTH_Acute_Finding'] = 'Acute'
        cth['CTH_Acute_Indeterminate'] = 'No'
        cth['CTH_Acute_Type'] = resolved_type
    elif mri_quality == 'Diagnostic':
        # Negative resolution: MRI was performed, is technically diagnostic (i.e. capable of
        # reliably excluding an acute finding), and found neither an acute infarct nor a
        # hemorrhage/other-abnormality of concern. The CTH equivocal call is now resolved
        # negative rather than left as a permanent open question. A Limited/Nondiagnostic MRI
        # does NOT resolve the indeterminate status, since it cannot reliably rule anything out.
        flags.append("CTH_Acute_Indeterminate resolved Yes->No (stays No Acute): "
                      "diagnostic follow-up MRI found no acute infarct or hemorrhage")
        cth['CTH_Acute_Indeterminate'] = 'No'
    return cth
